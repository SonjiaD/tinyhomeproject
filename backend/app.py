...
try:
    _poly_gdf = gpd.read_file("parking_polygons.geojson").to_crs(4326)
    _poly_list = []
    for _, _row in _poly_gdf.iterrows():
        _exterior = list(_row.geometry.exterior.coords[:-1])  # [(lon, lat), ...]
        _poly_list.append({
            "id":         str(_row["id"]),
            "parent_id":  str(_row["parent_id"]),
            "address":    str(_row.get("address", "")),
            "coordinates": [[lat, lon] for lon, lat in _exterior],  # [lat,lon] for Leaflet
        })
    POLY_RESPONSE = {"total_spots": len(_poly_gdf), "polygons": _poly_list}
    print(f"Loaded {len(_poly_gdf):,} parking polygons.")
except Exception as _e:
    print(f"Warning: parking_polygons.geojson not found or invalid ({_e}). Run generate_parking_polygons.py first.")
    POLY_RESPONSE = {"total_spots": 0, "polygons": []}
